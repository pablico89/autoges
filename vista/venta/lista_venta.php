...
<html lang="es">
  ...
  <body>
	...
	<div class="page-header">
	  <h1>Lista de ventas <small></small></h1>
	</div>
	<form action="#" method="get">
				 ...
				 <table class="table table-list-search table-striped table-bordered">
							...
							<tbody>
							 <?php $contador = 0; foreach ($lista_venta as $dato): $contador++;?>
										<tr>
										<td><?php echo $dato['id_venta']; ?></td>
										<td><?php echo $dato['fecha']; ?></td>
										<td><?php echo $dato['id_cliente']." ".$dato['nombre']; ?></td>
										<td><?php echo $dato['litotal']." €"; ?></td>
										<td><?php if($lista_saldo[$dato['id_venta']]['importe'] > 0) echo $lista_saldo[$dato['id_venta']]['importe']; else echo "0.00 €"; ?></td>
										<td><a href="autoventa.php?accion=imprime_venta&id_venta=<?php echo $dato['id_venta']?>" class="btn btn-default"><span class="glyphicon glyphicon-list-alt"></span> Imprimir</a></td>
										<td><a href="autoventa.php?accion=modifica_venta&miga=<?php echo $filtro;?>&id_venta=<?php echo $dato['id_venta'];?>" class="btn btn-default"><span class="glyphicon glyphicon-search"></span> Modificar</a></td>
										<td><?php if($lista_saldo[$dato['id_venta']]['importe'] > 0) echo '<a href="autoventa.php?accion=alta_cobro&id_venta='.$dato['id_venta'].'" class="btn btn-default"><span class="glyphicon glyphicon-euro"></span> Cobro</a>'?></td>
