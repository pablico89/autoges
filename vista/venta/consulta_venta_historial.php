	...
	<div class="modal fade" id="finaliza_venta" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	  <div class="modal-dialog">
		<div class="modal-content">
		  ...
		  <input type="hidden" name="accion" value="modifica_venta_historial">
  ...
  <body>
	...
	<a data-toggle="modal" id="boton_alta_linea" href="#lista_articulo" class="btn btn-primary"><span class="glyphicon glyphicon-plus-sign"></span> Añadir artículo</a>
	  ...
	  if( ($v['descuento_articulo']+$v['descuento_familia'] +$v['descuento']) != 0)
		...
		echo $v['pvp'];
		?></td>
	  <td class="alert" ><strong><?php echo $v['liquido_linea']." €"; ?></strong></td>
	  <td>
	  <a href="autoventa.php?accion=consulta_linea&miga=modifica&id_linea=<?php echo $v['id_linea']?>" class="btn btn-xs btn-info"><span class="glyphicon glyphicon-search"></span> Ver</a></td>
	  <td><a href="autoventa.php?accion=elimina_linea&miga=modifica&id_linea=<?php echo $v['id_linea']?>" class="btn btn-xs btn-danger"><span class="glyphicon glyphicon-trash"></span> </a></td>
	  </tr>
		...
		$( "#seleccion_articulo" ).click(function() {
			var id_articulo = $("#seleccion_id").val();
				$.ajax({
                    ...
                    success: function(data){
							if(data==1)
								$(location).attr('href','autoventa.php?accion=inserta_linea&miga=modifica&id_articulo='+$("#seleccion_id").val());                                    
							else
