...
$pdf->Ln(10);
	...
	foreach($datos_lineas as $dato_linea):  //iteramos en cada linea
	  ...
	  if($datos_cabecera['serie']=="N"){
		if($dato_linea["dcto1"] + $dato_linea["dcto2"]>0)
			...
			$precio2= number_format((($dato_linea['liquido']/$dato_linea['cantidad'])),2,",",".");
	  }else
	  ...
	  $total_unidades += $dato_linea["unidades"];
	endforeach;
	...
	$pdf->Cell(15,4,$total_cajas,1,0,'C',1);
	$pdf->Cell(15,4,$total_unidades,1,0,'C',1);
	$pdf->Ln(10);
	$pdf->Cell(30);
...
function imprime_carga($datos_cabecera, $datos_lineas){
...
$pdf->Ln(10);
    ...
    $pdf->Ln(4);
		$pdf->Cell(95);
		$pdf->Cell(95,4,"Almacen origen: ".$datos_cabecera["id_almacen_origen"],'LR',0,'L',1);
		$pdf->Ln(4);
		
		$pdf->Cell(95);
		$pdf->Cell(95,4,"Almacen destino: ".$datos_cabecera["id_almacen_destino"],'LR',0,'L',1);
		$pdf->Ln(4);
		
		$pdf->Cell(95);
		$pdf->Cell(95,4,"Total de cajas: ".$datos_cabecera["total_cajas"],'LR',0,'L',1);
		$pdf->Ln(4);
		
		$pdf->Cell(95);
		$pdf->Cell(95,4,"Total de unidades: ".$datos_cabecera["total_unidades"],'LR',0,'L',1);
		$pdf->Ln(4);
	...
	$fecha = date('d-m-y', $datos_cabecera['fecha']);
    $pdf->Cell(30,4,"CARGA No ".$datos_cabecera['id_carga'],0,0,'C',0);
	$pdf->Cell(30,4,"FECHA ".date("d-m-Y",strtotime($datos_cabecera['fecha'])),0,0,'C',0);
...
$pdf->Output($datos_cabecera['id_carga'].'.pdf',F);
}

function imprime_almacen($datos_cabecera, $datos_lineas){
